...
export const DEFAULT_MODEL = resolveAgentLlmModel();
...
export function pageUrlHash(url) {
	return createHash("sha256")
		.update(String(url || "").toLowerCase().trim())
		.digest("hex")
		.slice(0, 16);
}

/** True only for a real Maps place / lat,lng — not `query=CompanyName`. */
export function isRealMapsUrl(url) {
	const s = String(url || "");
	if (!s) return false;
	if (/\/maps\/place\//i.test(s)) return true;
	if (/[?&]cid=/i.test(s)) return true;
	if (/maps\.app\.goo\.gl|goo\.gl\/maps/i.test(s)) return true;
	if (/[?&]query=(-?\d+\.?\d*),(-?\d+\.?\d*)/.test(s)) return true;
	if (/[?&]q=(-?\d+\.?\d*),(-?\d+\.?\d*)/.test(s)) return true;
	return false;
}

export function hasResolvedLocation(data) {
	const loc = String(data?.locationStatus || "");
	...
	return isRealMapsUrl(data?.mapsUrl);
}

export function isSiteEnriched(data) {
	const status = String(data?.siteEnrichStatus || "");
	if (status === "error") return false;
	const locDone = hasResolvedLocation(data);
	if (status === "skipped") return locDone;
	const pages = data?.sitePages;
	const count =
		Number(data?.sitePageCount) ||
		(pages && typeof pages === "object" ? Object.keys(pages).length : 0) ||
		(Array.isArray(data?.sitePageUrls) ? data.sitePageUrls.length : 0);
	if (!(status === "done" && count > 0)) return false;
	return locDone;
}

export async function countCompanies(collection = YC_COLLECTION) {
	const snap = await firestore.collection(collection).count().get();
	return snap.data().count || 0;
}

export async function countSiteEnrichedCompanies(collection = YC_COLLECTION) {
	try {
		const snap = await firestore
			.collection(collection)
			.where("siteEnrichStatus", "in", ["done", "skipped"])
			.count()
			.get();
		return snap.data().count || 0;
	} catch {
		const d = await loadSiteEnrichCursor();
		return d.enriched || 0;
	}
}

export async function loadSiteEnrichCursor() {
	const snap = await firestore
		.collection(YC_STATE_COLLECTION)
		.doc(SITE_ENRICH_STATE_DOC)
		.get();
	const d = snap.data() || {};
	return {
...
export async function saveSiteEnrichCursor(patch) {
	await firestore
		.collection(YC_STATE_COLLECTION)
		.doc(SITE_ENRICH_STATE_DOC)
		.set(
			{
				agentId: SITE_ENRICH_STATE_DOC,
...
export async function updateCompanyDoc(id, patch, collection = YC_COLLECTION) {
...
/**
 * Walk yc-companies by document id, skip already site-enriched, return up to `limit`.
 */
export async function nextUnenrichedSiteBatch({ afterId = "", limit = 4 } = {}) {
	const picked = [];
	...
	while (picked.length < limit && !exhausted) {
		...
		for (const doc of snap.docs) {
			...
			scanned += 1;
			const data = doc.data() || {};
			if (isSiteEnriched(data)) continue;
