...
- Number thread tweets (1/ 2/ 3...) if multiple. Output the post(s) only. No JSON.`
	...
	"image-gallery-creator": {
		maxTokens: 4000,
		buildSystemPrompt: (_format, _style, _hasSources, opts = {}) => {
			const n = Math.min(10, Math.max(1, Number(opts.maxImages) || 6));
			return `You create an image gallery specification from the given content and prompt.
...
- Prefer real image URLs from the user message: (1) URLs listed under "Image URLs from page" from <img src>, (2) same-domain or absolute links that point directly to image files (.jpg, .png, .webp, .gif, etc.). If there are not enough distinct URLs, use placeholder URLs (e.g. https://placehold.co/600x400) and set alt/caption from the content.
- The "images" array must contain at most ${n} entries (fewer only if sources truly have fewer usable images).
- No markdown, no code fences — raw JSON only.`;
		},
		buildUserContent: (prompt, sources, opts = {}) => {
			const n = Math.min(10, Math.max(1, Number(opts.maxImages) || 6));
			const imgExt = /\.(jpg|jpeg|png|gif|webp|svg|avif)(\?|$)/i;
			const hrefOf = (l) => {
			...
			const collectImageUrls = (s) => {
				const fromImgs = (s.images || [])
					.map((im) => (typeof im === "string" ? im : im?.src || ""))
					.filter(Boolean);
				const fromLinks = (s.links || [])
					.map(hrefOf)
					.filter((u) => imgExt.test(u));
				return [...new Set([...fromImgs, ...fromLinks])];
			};
			if (!sources || sources.length === 0) {
				return `User request: ${prompt || "Create an image gallery."}\n\nProduce up to ${n} gallery entries. Use https://placehold.co/600x400 (or similar) for image URLs if you have no real URLs, with alt/caption matching the request.`;
			}
			const perSource = Math.min(
			...
			const blocks = sources.map((s, i) => {
				const urls = collectImageUrls(s);
				const urlLine = urls.slice(0, 40).join(", ") || "(none extracted)";
				return `--- Source ${i + 1}: ${s.url} ---\n${s.title ? `Title: ${s.title}\n` : ""}${(s.markdown || "").slice(0, perSource)}\nImage URLs from page (prefer these): ${urlLine}`;
			});
			return `User request: ${prompt || "Image gallery"}\n\nTarget: at most ${n} images in the output array.\n\nContent (use for context and image URLs):\n\n${blocks.join("\n\n")}`;
		},
		parseResponse: (raw) => {
	...
	// ─── Invoice generation (POST /generate/invoice) — format: json | markdown | react | html
	invoice: {
		maxTokens: 4096,
		buildSystemPrompt(format, _style, hasSources, _opts = {}) {
			const fmt = String(format || "json").trim().toLowerCase();

			const extractionCore = `
You are an expert invoice data extraction and generation engine.

Your job is to:
1. Parse any scraped content, URLs, or free-form prompt text to extract invoice-relevant information.
2. Infer and fill reasonable defaults for any missing fields (e.g. today's date, sequential invoice number like INV-001, NET-30 terms).
3. Calculate line-item subtotals, tax, discounts, and grand total yourself — never leave math to the client.
4. Never hallucinate company names, amounts, or line items that weren't implied by the input. If critical data is absent, use clearly marked placeholders like "[CLIENT NAME]".

Extract or infer these fields:
- Invoice metadata: invoice_number, issue_date, due_date, payment_terms, currency (default USD), status (draft|sent|paid|overdue)
- Sender (from): name, company, address (street, city, state, zip, country), email, phone, logo_url (if found in scraped images)
- Recipient (to): name, company, address, email, phone
- Line items[]: description, quantity, unit_price, discount_pct (0 if none), subtotal
- Totals: subtotal, discount_total, tax_rate, tax_amount, grand_total
- Notes / payment instructions (bank details, UPI, PayPal, etc.)
- Custom fields: any domain-specific fields found (PO number, project code, GST/VAT number, etc.)
`.trim();

			const formatInstructions = {
				json: `
OUTPUT FORMAT: JSON only. No markdown fences, no explanation, no preamble.
Return a single valid JSON object with this exact top-level shape:
{
  "invoice": {
    ...
    "currency": "USD",
    "status": "draft",
    "from": { "name":"","company":"","address":{},"email":"","phone":"","logo_url":"" },
    "to":   { "name":"","company":"","address":{},"email":"","phone":"" },
    "line_items": [
      { "description":"","quantity":1,"unit_price":0,"discount_pct":0,"subtotal":0 }
    ],
    "totals": { "subtotal":0,"discount_total":0,"tax_rate":0,"tax_amount":0,"grand_total":0 },
    "notes": "",
    "custom_fields": {}
  }
}
All numeric fields must be numbers (not strings). Dates in ISO-8601 (YYYY-MM-DD).
`.trim(),

				markdown: `
OUTPUT FORMAT: Clean, human-readable Markdown invoice document.
Structure it as:
...
**Subtotal:** $X  **Tax (X%):** $X  **Total Due:** $X

> Notes / Payment instructions

Keep it print-ready. Use proper currency formatting. No extra commentary outside the invoice.
`.trim(),

				react: `
OUTPUT FORMAT: A single self-contained React component (no imports needed except React).
Requirements:
- Named export + default export: export default function Invoice({ data }) { ... }
- Accept a \`data\` prop matching the JSON schema above (invoice object). Also embed sample hardcoded data as fallback if data prop is absent.
- Style with Tailwind CSS utility classes only (no external CSS files, no style tags).
- Design: clean, professional, print-ready. White background, sharp typography, clear visual hierarchy.
- Show all sections: header with logo placeholder, from/to blocks, line items table, totals breakdown, notes footer.
- Include a "Download PDF" button that calls window.print() with a print-friendly class toggle.
- All math already in data.totals — just render, don't recalculate.
- No external dependencies beyond React and Tailwind.
- Return only the component code. No markdown fences, no explanation.
`.trim(),

				html: `
OUTPUT FORMAT: A single, fully self-contained HTML file (inline CSS only, no external deps).
Requirements:
- Complete valid HTML5 document with <html>, <head>, <body>.
- Professional invoice layout — white card on light grey page background.
- Inline <style> block only. No external stylesheets, no CDN links.
- All invoice data hardcoded from the extracted fields — this is a rendered snapshot, not a template.
- Include a sticky "Print" button (top-right, disappears on @media print).
- @media print rules: remove button, expand card to full width, clean page breaks.
- Typography: system-ui or Georgia for body, clear size hierarchy for totals.
- Return only the HTML. No explanation, no markdown fences.
`.trim(),
			};

			const outputBlock =
				formatInstructions[fmt] ?? formatInstructions.json;

			const sourceNote = hasSources
				? `You have been provided scraped content from one or more URLs. Extract all invoice-relevant data from it first before falling back to the prompt.`
				: `No URLs were scraped. Rely entirely on the user's prompt to construct the invoice.`;

			return [extractionCore, sourceNote, outputBlock].join("\n\n");
		},
		buildUserContent(prompt, sources, _opts = {}) {
			const parts = [];

			if (sources.length > 0) {
				const srcBlock = sources
					.map((s, i) => {
						const imgList = (s.images || [])
							.slice(0, 5)
							.map((im) =>
			...
			return parts.join("\n\n") || "Generate a blank invoice template with placeholder values.";
		},
