...
const WELL_KNOWN_SITES = [
	{ pattern: /hacker[\s-]?news|news\.ycombinator\.com/i, url: "https://news.ycombinator.com" },
	...
	{ pattern: /ars[\s-]?technica/i, url: "https://feeds.arstechnica.com/arstechnica/index" },
	{ pattern: /smashing[\s-]?magazine/i, url: "https://www.smashingmagazine.com/feed" },
...
export function fastRouter(userPrompt, hasImages = false) {
	...
	const isTable = /\b(table|spreadsheet|csv|extract as table|create a table|give me table)\b/i.test(lp);
	const isBlog = /\b(blog post|write a blog|create a blog|\bblog\b)\b/i.test(lp);
	...
	const isGithubTrending = /\b(github trending|trending repos?|trending on github|trending repositories)\b/i.test(lp);
	...
	const buildContentTasks = (targetUrls, useCrawlResult = false) =>
		outputTypes.map((type) => ({
			type,
			label: `Create ${type}`,
			params: { urls: targetUrls, prompt, ...(useCrawlResult ? { useCrawlResult: true } : {}) },
		}));
	...
	// ── GitHub Trending ─────────────────────────────────────────────────────
	if (isGithubTrending) {
		const since = /\bdaily\b/i.test(lp) ? "daily" : /\bmonthly\b/i.test(lp) ? "monthly" : "weekly";
	...
	if (urls.length > 0) {
		const needsCrawl = /\b(crawl all|full site|sitemap|all pages?|deep crawl)\b/i.test(lp);
				...
				suggestedTasks: [
					{
						type: "crawl-url",
						label: "Crawl website",
						params: { url: urls[0], takeScreenshot: needsScreenshot, scrapeContent: true },
					},
	...
	return { confidence: 0, shouldExecute: false, suggestedTasks: [], thinking: "", message: "" };
}
