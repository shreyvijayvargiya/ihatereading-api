...
Respond with JSON only (no markdown fences):
{
  ...
  "suggestedTasks": [
    { "type": "crawl-url" | "scrape" | "scrape-git" | "github-trending" | "blog" | "article" | "table" | "newsletter" | "substack" | "linkedin" | "twitter" | "landing-page-generator" | "image-gallery-creator" | "infographics-svg-generator" , "label": "...", "params": { ... } }
...
- scrape-git: use ONLY for GitHub URLs (github.com). When the user provides a GitHub repo or file URL and needs code-level detail — e.g. "create a blog about this repo", "summarise this codebase", "what does this repo do", "read the code/imports" — suggest scrape-git with params.url set to the GitHub URL. This fetches the repo page plus AST (symbols, imports, exports) for AI context. Can be suggested alongside blog/article when the deliverable should use repo/code context; put the same GitHub URL in params.urls for the output task so it receives the scrape-git result. Optional params: includeStars (true to fetch star/forks count), includePullRequests (true to fetch open PRs with links), includeIssues (true to fetch open issues with links).
...
- Number thread tweets (1/ 2/ 3...) if multiple. Output the post(s) only. No JSON.`
	...
	// API-only: executed via POST /scrape-git. Use for GitHub URLs when code-level detail is needed (repo structure, symbols, imports, exports). AI gets markdown + compressed AST.
	"scrape-git": {
		maxTokens: 1,
		buildSystemPrompt: () => "N/A",
		buildUserContent: () => "N/A",
