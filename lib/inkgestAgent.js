...
const MAX_ASSET_FANOUT = 10;

/** Content skills the executor runs after a single shared scrape. */
const FANOUT_ASSET_TYPES = new Set([
	"blog",
	"article",
	"newsletter",
	"substack",
...
/** How many separate assets the user asked for (2–10), or 0 for one/default. */
export function parseRequestedAssetCount(prompt) {
	const lp = String(prompt || "").toLowerCase();
	const patterns = [
		/\b(\d{1,2})\s+(?:\w+\s+){0,4}?(?:blogs?|newsletters?|articles?|assets?)\b/,
		/\bgive\s+me\s+(\d{1,2})\s+(?:\w+\s+){0,3}?(?:blogs?|newsletters?|articles?|assets?)\b/,
		/\b(\d{1,2})\s+(?:blogs?|newsletters?|articles?|assets?)\s+in\s+total\b/,
		/\bin\s+total\s+(\d{1,2})\s+(?:blogs?|newsletters?|articles?|assets?)\b/,
	];
	for (const re of patterns) {
		const m = lp.match(re);
		if (m?.[1]) {
			const n = Number.parseInt(m[1], 10);
			if (n >= 2) return Math.min(n, MAX_ASSET_FANOUT);
		}
	}
	if (/\b(multiple|several)\s+(?:blogs?|newsletters?|articles?|assets?)\b/.test(lp))
		return 3;
	return 0;
}

function primaryAssetTypeFromPrompt(prompt) {
	const lp = String(prompt || "").toLowerCase();
	if (/\bnewsletters?\b/.test(lp)) return "newsletter";
	if (/\bsubstack\b/.test(lp)) return "substack";
	if (/\blinkedin\b/.test(lp)) return "linkedin";
	if (/\b(twitter thread|tweet thread)\b/.test(lp)) return "twitter";
	if (/\bblogs?\b/.test(lp)) return "blog";
	if (/\barticles?\b/.test(lp)) return "article";
	return "blog";
}

/**
 * When the user wants N assets but the router planned one content task, fan out to N tasks.
 * Scrape still runs once; each task invokes the same content skill (blog, newsletter, etc.).
 */
export function fanOutMultipleAssetTasks(tasks, userPrompt) {
	if (!Array.isArray(tasks) || tasks.length === 0) return tasks;

	const count = parseRequestedAssetCount(userPrompt);
	if (count < 2) return tasks;

	const assetType = primaryAssetTypeFromPrompt(userPrompt);
	const assetTasks = tasks.filter((t) => t?.type === assetType);
	if (assetTasks.length > 1) return tasks;

	const seed =
		assetTasks[0] ||
		tasks.find((t) => t.type === assetType) ||
		tasks.find((t) => FANOUT_ASSET_TYPES.has(t?.type));
	if (!seed) return tasks;

	const urls = [
		...(Array.isArray(seed.params?.urls) ? seed.params.urls : []),
	...
	const others = tasks.filter(
		(t) => t !== seed && !FANOUT_ASSET_TYPES.has(t?.type),
	);
	const basePrompt = seed.params?.prompt || userPrompt;

	return [
		...others,
		...Array.from({ length: count }, (_, i) => ({
			type: assetType,
			label: `${assetType} ${i + 1}/${count}`,
			params: {
				...seed.params,
				urls: uniqueUrls.length ? uniqueUrls : seed.params?.urls || [],
				prompt: `${basePrompt}\n\n(Create asset ${i + 1} of ${count} — a distinct ${assetType} from the other ${count - 1}; use the same scraped sources.)`,
				assetIndex: i + 1,
				assetTotal: count,
			},
		})),
...
export function buildAssetsFromExecuted(executed) {
	...
	return executed
		...
		.map((e) => ({
			...
			assetIndex: e.params?.assetIndex,
			assetTotal: e.params?.assetTotal,
		}));
}
...
export function fastRouter(userPrompt, hasImages = false) {
	...
	const assetCount = parseRequestedAssetCount(prompt);

	/** One content task per output type; fanOutMultipleAssetTasks splits into N later. */
	...
	const outputLabel =
		assetCount >= 2 && isBlog
			? `${assetCount} blogs${outputTypes.filter((t) => t !== "blog").length ? ` + ${outputTypes.filter((t) => t !== "blog").join(" + ")}` : ""}`
			: outputTypes.join(" + ");
...
async function scrapeUrlsViaApi(baseUrl, urls, options = {}) {
	...
	const perUrlPageTimeoutMs = aiSummary ? 90_000 : 60_000;
	const pageTimeout = Math.min(120_000, perUrlPageTimeoutMs);
	const body = {
		urls: validUrls,
		timeout: pageTimeout,
	...
	// Batch scrape: each URL may need navigation + evaluate + optional aiSummary LLM call.
	const perUrlBudgetMs = aiSummary ? 120_000 : 75_000;
	const SCRAPE_FETCH_TIMEOUT_MS = Math.min(
		600_000,
		30_000 + validUrls.length * perUrlBudgetMs,
	);
	...
	if (data.results && Array.isArray(data.results)) {
		...
		const sources = data.results
			...
			.filter((s) => s.markdown?.trim() || s.title?.trim());
		const errors = data.results
			.filter((r) => r.success === false)
			.map((r) => ({ url: r.url || "unknown", error: r.error || "Scrape failed" }));
...
MULTIPLE ASSETS - When the user asks for 2+ deliverables (e.g. "give me 4 blogs", "3 newsletters", "multiple articles based on these URLs"):
- Suggest MULTIPLE separate content tasks (one per asset) using the SAME skill type (blog, newsletter, article, etc.) — NOT one combined output.
- Do NOT add extra scrape tasks; put all URLs in params.urls on every content task. Scraping runs once before content tasks execute.
- Each task: distinct label, params.prompt with a unique angle for that asset, params.assetIndex (1-based) and params.assetTotal.
- Max ${MAX_ASSET_FANOUT} content tasks of the same type. Example: "4 blogs from https://a.com and https://b.com" → 4 blog tasks, each params.urls: [both URLs], different params.prompt per blog.`;
