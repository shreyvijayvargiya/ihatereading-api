...
/** Deliverable types surfaced in the final `assets` array for the frontend. */
export const INKGEST_DELIVERABLE_TYPES = new Set([
	"blog",
	"article",
	"newsletter",
	"substack",
...
/**
 * Parse multi-blog intent from a user prompt (2–10 distinct blog posts).
 * @returns {{ count: number, topics: string[] } | null}
 */
export function detectMultiBlogIntent(prompt) {
	const text = String(prompt || "");
	const lp = text.toLowerCase();
	if (!/\b(blogs?|blog\s+posts?)\b/i.test(lp)) return null;

	let count = null;

	const countPatterns = [
		/\b(\d{1,2})\s+(?:separate\s+)?(?:different\s+)?(?:blog\s+posts?|blogs)\b/,
		/\b(?:give\s+me|get\s+me|i\s+need|want|write|create)\s+(\d{1,2})\s+blogs?\b/,
		/\b(\d{1,2})\s+blogs?\s+in\s+total\b/,
		/\bin\s+total\s+(\d{1,2})\s+blogs?\b/,
		/\b(\d{1,2})\s+blogs?\s+total\b/,
	];
	...
	if (!count) {
		for (const [word, n] of Object.entries(WORD_NUMBER_TO_INT)) {
			if (
				n >= 2 &&
				new RegExp(
					`\\b(?:give\\s+me\\s+|get\\s+me\\s+)?${word}\\s+(?:separate\\s+)?blogs?\\b`,
				).test(lp)
			) {
				count = n;
				break;
	...
	if (!count && /\b(multiple|several|few)\s+blogs?\b/.test(lp)) count = 3;

	const topics = extractBlogTopicsFromPrompt(text);
	if (!count && topics.length >= 2) count = topics.length;

	if (!count || count < 2 || !Number.isFinite(count)) return null;

	return {
		count: Math.min(Math.max(2, count), MULTI_BLOG_MAX),
		topics,
	};
}

/** Hostname label for multi-blog angles when topics are not listed explicitly. */
...
/** Distinct angle per blog when the user only gives a count + source URLs. */
function deriveMultiBlogAngles(userPrompt, targetUrls, count, topics) {
	const angles = [...topics];
	const hosts = [
		...new Set(
			(targetUrls || [])
	...
	if (hosts.length > 0) {
		hosts.forEach((host) => {
			if (angles.length < count) {
				angles.push(
					`In-depth blog focused on ${host}: what it offers, positioning, audience, and key takeaways`,
				);
			}
		});
		if (hosts.length >= 2 && angles.length < count) {
			angles.push(
				`Head-to-head comparison of ${hosts.join(" vs ")} — strengths, differences, and who each is for`,
			);
		}
		if (angles.length < count) {
			angles.push(
				`Round-up synthesis using ${hosts.join(", ")}: trends, patterns, and actionable advice for builders`,
			);
		}
	}
	...
	const fallbacks = [
		...
		"Product features, templates, and developer experience",
		"Go-to-market, positioning, and differentiation",
		"Practical guide, checklist, and recommendations",
		"Case-style walkthrough with concrete examples",
		"Future outlook, trends, and next steps",
	];
	while (angles.length < count) {
		const n = angles.length + 1;
		angles.push(
			`${fallbacks[angles.length % fallbacks.length]} (installment ${n} of ${count})`,
		);
	}
...
/** Pull explicit per-blog topics from lists in the prompt. */
function extractBlogTopicsFromPrompt(prompt) {
	const topics = [];

	const numbered = [...prompt.matchAll(/^\s*\d+[\.)]\s*(.+)$/gm)];
	if (numbered.length >= 2) {
		return numbered
			.map((m) => m[1].trim().replace(/\.$/, ""))
			.filter((t) => t.length > 2)
			.slice(0, MULTI_BLOG_MAX);
	}

	const listPatterns = [
		/\bblogs?\s+for\s+(?:my\s+)?[^:]+:\s*([^.\n]+)/i,
		/\bblogs?\s+for\s+([^.\n]+)/i,
		/\bblogs?\s+about\s+([^.\n]+)/i,
		/\bblogs?\s*:\s*([^\n]+)/i,
		/\bcreate\s+blogs?\s+for\s+([^.\n]+)/i,
	];
	...
	return [...new Set(topics)].slice(0, MULTI_BLOG_MAX);
}

/** Fan out N blog tasks — each with a distinct angle/topic. */
export function buildMultiBlogTasks(
	userPrompt,
	targetUrls,
	multi,
	useCrawlResult = false,
) {
	const { count, topics } = multi;
	const angles = deriveMultiBlogAngles(userPrompt, targetUrls, count, topics);
	const tasks = [];
	for (let i = 0; i < count; i++) {
		const angle = angles[i];
		const blogPrompt = `MULTI-BLOG SERIES — write blog ${i + 1} of ${count} only.

This installment's exclusive focus: ${angle}

Rules for this post:
- Complete, standalone SEO blog (intro, 3–5 H2/H3 sections, conclusion/CTA).
- Must NOT repeat the same structure or thesis as the other ${count - 1} posts in this series.
- Use scraped source URLs as research; cite with backlinks. Do not invent facts.
- Full user request for context: ${userPrompt}`;

		const shortLabel = angle.replace(/^In-depth blog focused on /i, "").slice(0, 72);
		tasks.push({
			type: "blog",
			label: `Blog ${i + 1}/${count}: ${shortLabel}`,
			params: {
				urls: targetUrls,
				prompt: blogPrompt,
				multiBlog: true,
				multiBlogIndex: i + 1,
				multiBlogTotal: count,
				multiBlogAngle: angle,
...
const MULTI_BLOG_FANOUT_REPLACE_TYPES = new Set(["blog", "article"]);

/**
 * Replace a single content task (blog or article) with N blog tasks when the prompt asks for multiple blogs.
 * Skips when multiple blog tasks are already present.
 */
export function expandMultiBlogInSuggestedTasks(tasks, userPrompt) {
	if (!Array.isArray(tasks) || tasks.length === 0) return tasks;
	const multi = detectMultiBlogIntent(userPrompt);
	if (!multi) return tasks;

	const blogTasks = tasks.filter((t) => t?.type === "blog");
	if (blogTasks.length > 1) return tasks;

	const seed =
		blogTasks[0] ||
		tasks.find((t) => MULTI_BLOG_FANOUT_REPLACE_TYPES.has(t?.type));

	const withoutContentDeliverables = tasks.filter(
		(t) => !MULTI_BLOG_FANOUT_REPLACE_TYPES.has(t?.type),
	);

	const urls = [
		...(seed && Array.isArray(seed.params?.urls) ? seed.params.urls : []),
		...collectHttpUrlsFromTasks(tasks),
	...
	const useCrawlResult = seed?.params?.useCrawlResult === true;

	if (!seed && uniqueUrls.length === 0) return tasks;

	return [
		...withoutContentDeliverables,
		...buildMultiBlogTasks(userPrompt, uniqueUrls, multi, useCrawlResult),
	];
}

/** Normalise executed deliverables into a flat array for frontend `.map()`. */
export function buildAssetsFromExecuted(executed) {
	if (!Array.isArray(executed)) return [];
	return executed
		.filter((e) => e && INKGEST_DELIVERABLE_TYPES.has(e.type))
		.map((e) => ({
			type: e.type,
			label: e.label,
			content:
				e.content ??
				e.html ??
				(Array.isArray(e.infographics)
					? JSON.stringify(e.infographics)
			...
			html: e.html,
			result: e.result,
			infographics: e.infographics,
			images: e.images,
			condensedSources: e.condensedSources,
			sourceUrls: e.sourceUrls,
			multiBlogIndex: e.params?.multiBlogIndex,
			multiBlogTotal: e.params?.multiBlogTotal,
		}));
}
...
export function fastRouter(userPrompt, hasImages = false) {
	...
	const multiBlogEarly = detectMultiBlogIntent(prompt);
	const isBlog =
		/\b(blogs?|blog\s+posts?|write\s+(?:a\s+)?blogs?|create\s+(?:a\s+)?blogs?)\b/i.test(
			lp,
		) || !!multiBlogEarly;
	...
	const multiBlog = multiBlogEarly || detectMultiBlogIntent(prompt);
	// "4 blogs" + URLs must route to blog tasks, not a single article
	if (multiBlog && !outputTypes.includes("blog")) {
		const withoutArticle = outputTypes.filter((t) => t !== "article");
		outputTypes.length = 0;
		outputTypes.push("blog", ...withoutArticle);
	}

	/** Build content tasks; fans out to N blog tasks when multi-blog intent is detected. */
	const buildContentTasks = (targetUrls, useCrawlResult = false) => {
		const tasks = [];
		for (const type of outputTypes) {
			if (type === "blog" && multiBlog) {
				tasks.push(
					...buildMultiBlogTasks(prompt, targetUrls, multiBlog, useCrawlResult),
				);
			} else {
				tasks.push({
					type,
	...
	const outputLabel = multiBlog
		? `${multiBlog.count} blogs${outputTypes.filter((t) => t !== "blog").length ? ` + ${outputTypes.filter((t) => t !== "blog").join(" + ")}` : ""}`
		: outputTypes.join(" + ");
...
MULTIPLE BLOGS - When the user asks for 2+ blogs (e.g. "give me 4 blogs in total", "create 3 blogs", "multiple blogs for landing, menu, about", numbered list of blog topics):
- Suggest MULTIPLE separate blog tasks (one per blog), NOT one combined blog or article task.
- Put ALL URLs from the prompt into every blog task's params.urls (e.g. "4 blogs based on https://a.com and https://b.com" → 4 blog tasks, each with params.urls: [both URLs]).
- Each task: type "blog", distinct label (e.g. "Blog 2/4: …"), params.prompt with a unique angle for that installment, params.multiBlogIndex and params.multiBlogTotal.
- Max ${MULTI_BLOG_MAX} blog tasks. If the user lists topics (comma-separated or numbered), one task per topic; otherwise split angles per source URL (per-site deep dive, comparison, synthesis).`;
...
Multi-blog series: when the user message says "MULTI-BLOG SERIES" or "blog N of M", you are writing ONE installment only. Use a unique angle for this installment; do not merge multiple posts into one output.
...
Structure: intro, sections, conclusion. Include links and images where relevant.
When the user message says "MULTI-BLOG SERIES" or "blog N of M", write only that installment with a distinct angle.
