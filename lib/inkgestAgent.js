...
export function detectMultiBlogIntent(prompt) {
	...
	const digitMatch = lp.match(
		/\b(\d{1,2})\s+(?:separate\s+)?(?:different\s+)?(?:blog\s+posts?|blogs)\b/,
	);
	if (digitMatch) count = Number.parseInt(digitMatch[1], 10);

	if (!count) {
		for (const [word, n] of Object.entries(WORD_NUMBER_TO_INT)) {
			if (n >= 2 && new RegExp(`\\b${word}\\s+(?:separate\\s+)?blogs?\\b`).test(lp)) {
				count = n;
				break;
			}
		}
...
export function buildMultiBlogTasks(
	...
	for (let i = 0; i < count; i++) {
		...
		const blogPrompt = topic
			? `Write a complete, standalone SEO blog post focused on: ${topic}. Full user context: ${userPrompt}`
			: `Write blog post ${i + 1} of ${count}. This must be a distinct, standalone post (different angle/section than the others). User request: ${userPrompt}`;
		tasks.push({
			type: "blog",
			label: topic ? `Blog: ${topic.slice(0, 72)}` : `Blog ${i + 1}`,
			params: {
				urls: targetUrls,
				prompt: blogPrompt,
				multiBlog: true,
...
/**
 * Replace a single router `blog` task with N blog tasks when the prompt asks for multiple blogs.
 * Skips when the client already passed multiple blog tasks via executeTasks.
 */
export function expandMultiBlogInSuggestedTasks(tasks, userPrompt) {
	...
	const blogTask = blogTasks[0];
	const others = tasks.filter((t) => t?.type !== "blog");
	const urls = Array.isArray(blogTask.params?.urls) ? blogTask.params.urls : [];
	const useCrawlResult = blogTask.params?.useCrawlResult === true;

	return [
		...others,
		...buildMultiBlogTasks(userPrompt, urls, multi, useCrawlResult),
	];
}
...
export function fastRouter(userPrompt, hasImages = false) {
	...
	const multiBlog = isBlog ? detectMultiBlogIntent(prompt) : null;
...
async function scrapeUrlsViaApi(baseUrl, urls, options = {}) {
	...
	if (validUrls.length === 1 && data.success) {
		// Legacy /scrape-shaped response (direct call or older proxies)
		const markdown =
...
MULTIPLE BLOGS - When the user asks for 2+ blogs (e.g. "create 3 blogs", "multiple blogs for landing, menu, about", numbered list of blog topics):
- Suggest MULTIPLE separate blog tasks (one per blog), NOT one combined blog task.
- Each task: type "blog", distinct label (e.g. "Blog: Menu page"), params.prompt describing that specific blog's focus, same params.urls for shared scrape context.
- Set params.multiBlogIndex (1-based) and params.multiBlogTotal on each blog task.
- Max ${MULTI_BLOG_MAX} blog tasks. If the user lists topics (comma-separated or numbered), one task per topic.`;
