...
async function planLookups(doc, useAI = false, model) {
...
async function mergeWithEvidence(doc, evidence, useAI = false, model) {
...
export async function runAiStylesEnrichAgent(opts = {}) {
	...
	for (const doc of rows) {
		const id = doc.id;
		try {
			const plan = await planLookups(doc, useAI, model);
			...
			const decision = await mergeWithEvidence(doc, evidence, useAI, model);
