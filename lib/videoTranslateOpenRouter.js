...
import { resolveTranslateTtsVoice } from "./translateTtsVoices.js";

/** Load .env before any process.env reads — index.js runs dotenv after imports, so without this, VIDEO_TRANSLATE_* may be unset when resolving yt-dlp. */
const __dirname_lib = path.dirname(fileURLToPath(import.meta.url));
dotenv.config({ path: path.join(__dirname_lib, "..", ".env") });
...
/**
 * Prefer system/Homebrew yt-dlp (Python ≥3.10). The bundled binary in youtube-dl-exec may run on macOS
 * Command Line Tools Python 3.9 and fail. Optional: VIDEO_TRANSLATE_YTDLP_BINARY, else common paths.
 */
function resolveYoutubeDlBinaryPath() {
	const explicit = process.env.VIDEO_TRANSLATE_YTDLP_BINARY?.trim();
	if (explicit) {
		try {
		...
		console.warn(
			`[video-translate] VIDEO_TRANSLATE_YTDLP_BINARY not found at "${explicit}" (check Apple Silicon vs Intel: /opt/homebrew vs /usr/local). Trying common paths.`,
		);
	}
...
async function downloadYoutubeViaYtDlp(url, signal, tmpDir) {
	const ytDlpPath = resolveYoutubeDlBinaryPath();
	const dl = ytDlpPath ? createYoutubeDl(ytDlpPath) : youtubedl;
	if (!ytDlpPath) {
		console.warn(
			"[video-translate] Using bundled yt-dlp from youtube-dl-exec; requires Python 3.10+. Install: brew install yt-dlp, or set VIDEO_TRANSLATE_YTDLP_BINARY.",
		);
	}
 ...
 */
async function ttsSingleAttempt({ text, model, format, signal, voice }) {
 ...
 * @param {{ text: string, signal?: AbortSignal, voice?: string }} opts
 */
export async function ttsSynthesizeToBuffer({ text, signal, voice }) {
...
export async function getVideoTranslateJobStatus(videoTranslateId) {
	...
	const heygenLike = {
		...
		tts_voice: d.tts_voice ?? null,
		tts_voice_preset: d.tts_voice_preset ?? null,
		status:
...
export async function createVideoTranslateJobs({ videoUrl, body }) {
	...
	const vr = resolveTranslateTtsVoice(body.voice ?? body.tts_voice);
	if (!vr.ok) {
		return {
			error: vr.error,
			code: "BAD_REQUEST",
			details: { allowed_voices: vr.allowed },
			httpStatus: 400,
		};
	}
