...
async function downloadVideoToTempFile(url, signal, maxVideoBytes, maxMinutes) {
	const requiredTmpBytes = Math.floor(
		maxVideoBytes * Math.max(1, TMP_SPACE_MULTIPLIER) + TMP_SPACE_BASE_BUFFER_BYTES,
	);
	await ensureTmpHasSpace(requiredTmpBytes, "video download/transcode");
