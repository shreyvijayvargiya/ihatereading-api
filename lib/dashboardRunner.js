...
function ingestStats(state, line) {
	...
	if (trimmed.startsWith("{") && trimmed.endsWith("}")) {
		try {
			...
			if (j.agentId === "england-clubs-enrich" && j.done) {
				state.loop = false;
				pushLog(
					state,
					"sys",
					`enrichment finished ${j.enriched || 0}/${j.stored || 0} clubs — loop off`,
				);
			}
			if (j.agentId === "yc-companies-site-enrich" && j.done) {
				state.loop = false;
				pushLog(
					state,
					"sys",
					`site enrich finished ${j.enriched || 0}/${j.stored || 0} companies — loop off`,
				);
			}
		} catch {
			/* ignore */
