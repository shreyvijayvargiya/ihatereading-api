...
import { generateImageUsingOpenAIServerLocally } from "./mlxOpenAiImage.js";
...
async function generateCarouselImage(prompt, { imageBackend } = {}) {
...
Instagram carousel slide, ${IG_ASPECT} portrait ${IG_SIZE}, bold modern graphic, high contrast, large readable headline, no watermark, no platform UI, no extra captions.`;
	...
	const preferMlx = imageBackend !== "openrouter";

	if (preferMlx) {
		try {
			return await generateImageUsingOpenAIServerLocally({
				prompt: fullPrompt,
				size: process.env.MLX_IMAGE_SIZE?.trim() || "1024x1280",
			});
		} catch (err) {
			if (imageBackend === "mlx") throw err;
			console.warn(
				"[ig-carousel] local mlx-openai-server failed, falling back to OpenRouter:",
				err?.message || err,
			);
		}
	}
	...
	if (fromImages) {
		return { ...dataUrlToBuffer(fromImages), backend: "openrouter" };
	}
	...
	for (const part of parts) {
		...
		if (String(url).startsWith("data:")) {
			return { ...dataUrlToBuffer(url), backend: "openrouter" };
		}
...
/**
 * @param {{ url: string, slides?: number, style?: string, baseUrl?: string, c?: object, imageBackend?: string }} opts
 */
export async function generateUrlIgCarousel(opts = {}) {
	...
	const imageBackend = String(opts.imageBackend || process.env.IG_CAROUSEL_IMAGE_BACKEND || "auto")
		.trim()
		.toLowerCase();
	const imageConcurrency =
		imageBackend === "openrouter" ? IMAGE_CONCURRENCY : 1;
	...
	const rendered = await mapPool(plan.slides, imageConcurrency, async (slide) => {
		try {
			const { buffer, mime, backend } = await generateCarouselImage(
				slide.imagePrompt ||
					`Instagram cover slide, bold headline: ${slide.headline}`,
				{ imageBackend },
			);
			const imageUrl = await uploadPngToUploadThing(buffer, mime, slide.index);
			return { ...slide, imageUrl, imageError: null, imageBackend: backend || imageBackend };
		} catch (err) {
			return {
				...slide,
				imageUrl: null,
	...
	return {
		...
		models: {
			...
			image:
				rendered.some((s) => s.imageBackend === "openrouter")
					? IG_CAROUSEL_IMAGE_MODEL
					: "mlx-openai-server",
		},
