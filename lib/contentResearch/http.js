...
export async function googleSearch(query, opts = {}) {
	...
	// 3) DuckDuckGo HTML — fast, no Chrome
	try {
	...
	if (opts.skipPuppeteer) return [];

	// 4) Local /google-search (Puppeteer) — last resort, often 90s+
	for (const useProxy of [Boolean(opts.useProxy), false]) {
		...
		try {
			const res = await fetch(`${baseUrl}/google-search`, {
				method: "POST",
				signal: AbortSignal.timeout(opts.puppeteerTimeoutMs || 45_000),
...
/**
 * Firecrawl page scrape — works when local POST /scrape is down or blocked.
 * Env: FIRECRAWL_API_KEY
 */
export async function firecrawlScrape(url) {
	...
	if (!target) return { error: "empty_url" };
	...
	try {
		...
		const data = await res.json().catch(() => ({}));
		if (!res.ok || data.success === false) {
			return { error: data.error || `Firecrawl scrape HTTP ${res.status}` };
		}
		const row = data.data || data;
		const links = Array.isArray(row.links)
			? row.links.map((l) => (typeof l === "string" ? l : l?.url || l?.href || "")).filter(Boolean)
			: [];
		return {
			title: row.metadata?.title || row.title || "",
			markdown: String(row.markdown || "").slice(0, 80_000),
			html: String(row.html || "").slice(0, 80_000),
			links,
			error: null,
		};
	} catch (err) {
