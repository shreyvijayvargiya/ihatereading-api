...
let _browserAgentSparticuzPathPromise = null;

async function launchBrowserAgentChromium() {
	const baseArgs = [
		"--no-sandbox",
	...
	const chromiumMod = (await import("@sparticuz/chromium")).default;
	if (!_browserAgentSparticuzPathPromise) {
		_browserAgentSparticuzPathPromise = chromiumMod.executablePath();
	}
	const executablePath = await _browserAgentSparticuzPathPromise;
	try {
		return await puppeteer.launch({
			headless: true,
			executablePath,
			args: [...chromiumMod.args, ...baseArgs],
...
async function runJob(jobId, input) {
	...
	try {
		...
		browser = await launchBrowserAgentChromium();
